...
## S12 真实 executor adapter：sandbox(OpenSandbox) + MCP（§6.23）
...
## S13 执行层织入圆桌 turn（工具化发言，§6.24）

> 不改产品逻辑：圆桌/配方/调度/human-gate/点账本/synthesize 全不动。每个 AI 的一轮发言可先用工具（沙箱跑代码 / MCP）干活=参与者的"手"，再走**现有流式人设发言**（β）。trace 按 (group_key, speaker, turn) 归属，圆桌 SSE 实时流执行子事件，ChatPage 点进 AI 看"查看执行"抽屉（嵌 ExecutionPage）。
> **门控**：仅当 server 配了 sandbox（execution 启用）时 turn 才跑 loop；未配=今天的纯发言（现有测试零改、单机 pip 路径不变）。复用 S11/S12 全部执行机器（plan→tool_dispatch→loop_guard、真 executor）。
...
S11 P1 ─→ S12(真实 executor adapter: S12a 协议+Fake → S12b OpenSandbox → S12c 复用/探活 → S12d MCP → S12e service wire)
S12 + S3.x(圆桌) ─→ S13(执行织入 turn，§6.24: S13a 真 plan stream → S13b turn 织 loop(β/门控) → S13c trace 存+SSE → S13d server 通电 → S13e ChatPage 抽屉 → S13f MCP 注册表)
```
