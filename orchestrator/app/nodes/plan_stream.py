...
_PLAN_SYSTEM = """你是一个能在隔离沙箱里运行命令的助手。每一步**只输出一个 JSON 对象**，\
不要任何额外文字、解释或 ``` 围栏。
...
可用工具：
- sandbox_exec：在隔离沙箱里执行一条 shell 命令（args.command）。跑 Python 用 `python3 -c "..."`。
...
def _build_plan_messages(state: GroupState) -> list:
    ...
    return [SystemMessage(content=_PLAN_SYSTEM), HumanMessage(content=user)]


def default_plan_stream(model) -> PlanStream:
    """Real planner: build the ReAct prompt, ask the model for the next JSON intent."""

    async def stream(state: GroupState) -> AsyncIterable[str]:
        msg = await robust_ainvoke(model, _build_plan_messages(state))
